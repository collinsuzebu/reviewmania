...
def total_videos(directory):
	total = [m for m in os.listdir(directory) 
				if is_supported(m)
			 ]
...
def printf(string, color="reset"):
	r = f"{SUPPORTED_FORMATS['colors'][color]}"\
		f"{string}"\
...
def clean_title(movie_title, yr=False):
	...
	try:
		...
		movie_title = [' '.join(movie_title[:movie_title.index(word)])
						for word in movie_title 
							if word.lower() in stopwords][0]
	except IndexError:
	...
	if year:
		movie_title = movie_title[:movie_title.index(year.group(1))] + "(" + movie_title[movie_title.index(year.group(1)[0]):]
	else:
	...
	if "(" in movie_title: # Check to see if common pattern such as '(' exist in string
		start_delete = movie_title.index('(') # If it exist, it's additional data can be safely removed
...
class FileWriter:
	...
	def __init__(self, filename):
		...
		self.file_path = os.path.join(os.path.normpath(os.path.expanduser("~/Desktop")), filename)
		self.file_path2 = os.path.join(os.path.normpath(os.path.expanduser("~/Desktop")), 'final_'+filename)
	...
	def create_text_file(self, movie_list):
		df = pd.DataFrame(movie_list, columns=['movie_name', 'movie_year', 'movie_review']) # Create a pandas dataframe
		df.index =df.index + 1 # Set index to start counting from 1
		export_df = df.to_csv(self.file_path2, sep='\t', index=True, encoding='utf-8', header=True) # Export file to text